...
const style = {
  ...
  actionItemsContainer: `flex cursor-pointer items-center justify-center p-3`,
...
const NFTBasicInfo = ({name}) => {
  ...
  return (
    <div>
      <div className={style.topContainer}>
        ...
        <div className={style.actionItems}>
          {actionItems.map((item, index) => (
            <div key={index} className={style.actionItemsContainer}>
              {item.icon}
               </div>
          ))}
        </div>
      ...
      <div className={style.subInfoContainer}><NFTSubInfo /></div>
    </div>
  )
}
